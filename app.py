...
APP_BUILD = "PROXIMA V7 — Regression-only final"
...
st.set_page_config(
    page_title="PROXIMA Trueness Regression",
    page_icon="🧪",
    layout="wide",
)

st.title("🧪 PROXIMA Trueness Regression App")
st.caption(f"Build: {APP_BUILD}")
st.caption(
    "Regression-only implementation of the validated PROXIMA trueness notebook: "
    "global-flag filtering, replicate-level generalized ESD, donor averaging, "
    "Huber regression, Pearson r with Fisher-z 95% CI, full audits, Excel export, "
    "and all regression plot outputs."
)

with st.expander("Method lock and interpretation", expanded=False):
    st.markdown(
        """
- **Validated trueness branch:** `global_flag == FALSE` → generalized ESD on raw linked replicate rows → analyte-specific replicate removal → donor means → Huber regression → Pearson *r* with Fisher-z 95% CI.
- **Optional sensitivity run:** the user may disable replicate-level outlier removal; all otherwise eligible replicates are then retained before donor averaging. Global and analyte-specific flag exclusions still apply.
- **Donor parsing:** the app extracts the D-token, collection prefix, and optional explicit Donor column, then asks the user which identity rule to use. This prevents accidental merging when labels such as `D03` and `D03b` are distinct.
- **Specimen selection:** Mc, Mc2, MK2, MK3, Dir, Tas/Tasso, and unknown uploaded specimen labels can be selected independently.
- **Regression outputs:** Huber slope/intercept and equation, Pearson *r*, Fisher-z lower and upper 95% CI, donor count, observed reference and MHS ranges, replicate counts, outlier audit, and normal-range boundary lines.

Use de-identified study data only. AC/CLIA fields remain editable contextual metadata for the regression outputs.
"""
    )
...
st.caption("This final build runs the validated trueness-regression workflow only.")

run_clicked = st.button("Run PROXIMA trueness regression", type="primary", use_container_width=True)
if run_clicked:
    try:
        ...
        with st.status("Running the validated trueness-regression pipeline…", expanded=True) as status:
            st.write("Applying donor/specimen selection and global_flag = FALSE filtering…")
            st.write(
                "Running replicate-level generalized ESD before donor averaging…"
                if apply_outlier_removal else
                "Running regression with replicate-level outlier removal disabled…"
            )
            st.write("Computing Huber regression and Pearson r with Fisher-z 95% CI…")
            st.write("Saving regression plots, CSV audits, Excel workbook, and ZIP output…")

            with tempfile.TemporaryDirectory(prefix="proxima_regression_app_") as temp_dir:
                pipeline_kwargs = {
                ...
                if unsupported_kwargs:
                    raise RuntimeError(
                        "app.py and analysis_core.py are mismatched. Replace both V7 files together. "
                        "Unsupported core arguments: " + ", ".join(unsupported_kwargs)
                    )

                result = run_complete_notebook_pipeline(canonical, temp_dir, **pipeline_kwargs)
                ...
                replicate_counts = pd.concat(
                    [bundle["replicate_counts"] for bundle in result["regression"]["groups"].values()],
                    ignore_index=True,
                ) if result["regression"]["groups"] else pd.DataFrame()
                ...
                st.session_state["proxima_regression_results"] = {
                    ...
                    "global_exclusions": result["regression"]["global_exclusions"].copy(),
            ...
            status.update(label="Regression analysis complete", state="complete", expanded=False)
        st.success("The regression-only PROXIMA analysis completed successfully.")
    except Exception as exc:
        st.exception(exc)


results = st.session_state.get("proxima_regression_results")
if results is None:
    st.stop()
...
summary5.metric("Replicate outliers removed", len(results["regression_outliers"]))
st.caption(
...
reg_tab, audit_tab, plot_tab, download_tab = st.tabs([
    "Trueness regression",
...
with download_tab:
    ...
    d1.download_button(
        ...
        results["zip"],
        file_name="PROXIMA_REGRESSION_RESULTS.zip",
    ...
    d2.download_button(
        ...
        results["xlsx"],
        file_name="PROXIMA_Trueness_Regression_Output.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        use_container_width=True,
    )
    d3.download_button(
        "Download regression summary CSV",
        results["summary_csv"],
        file_name="PROXIMA_REGRESSION_SUMMARY.csv",
        mime="text/csv",
        use_container_width=True,
    )
    d4.download_button(
        "Download canonical selected data",
        results["canonical_csv"],
        file_name="PROXIMA_CANONICAL_SELECTED_DATA.csv",
        mime="text/csv",
        use_container_width=True,
    )
    with st.expander("Output inventory", expanded=False):
