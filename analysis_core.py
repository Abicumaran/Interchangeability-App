"""Core statistical engine for the PROXIMA regression-only Streamlit app.

The active public pipeline implements the validated APT trueness workflow:
global_flag == FALSE filtering, replicate-level generalized ESD, donor averaging,
Huber regression, Pearson correlation, and Fisher-z 95% confidence intervals.
Bland–Altman execution is intentionally disabled in this build.
"""
...
def compute_reference_ba_results(
    ...
    for analyte, cfg in analytes.items():
        ...
        pct_summary = ba_summary(pct_values, rng)
        native_summary = ba_summary(native_values, rng)
        normal_low, normal_high = map(float, cfg["normal"])
        midpoint = (normal_low + normal_high) / 2.0
...
# =============================================================================
# V7 REGRESSION-ONLY PUBLIC PIPELINE
# This definition intentionally overrides the historical combined entry point.
# =============================================================================
def run_complete_notebook_pipeline(
    canonical_data: pd.DataFrame,
    run_root: str | Path,
    ...
    **compatibility_kwargs: object,
    ...
    """Run only the validated PROXIMA trueness-regression workflow.

    Bland–Altman is not accepted, configured, validated, or executed by this
    public V7 entry point. Global-flag filtering remains mandatory inside
    ``run_pipeline`` regardless of the replicate-outlier sensitivity option.
    """
    alias_map = {
        "equation_position": "regression_equation_position",
        "metrics_position": "regression_metrics_position",
    ...
    # Explicitly reject any stale BA arguments so mixed deployments fail clearly.
    stale_ba = sorted(key for key in compatibility_kwargs if "bland" in key.lower() or key.lower().startswith("ba_"))
    if stale_ba:
        raise TypeError(
            "This is the PROXIMA V7 regression-only core. Remove Bland-Altman arguments from app.py: "
            + ", ".join(stale_ba)
        )
    if compatibility_kwargs:
        raise TypeError(
            "run_complete_notebook_pipeline() received unsupported keyword argument(s): "
            + ", ".join(sorted(compatibility_kwargs))
        )
    ...
    run_root = Path(run_root)
    run_root.mkdir(parents=True, exist_ok=True)
    output_dir = run_root / "PROXIMA_REGRESSION_RESULTS"
    trueness_dir = output_dir / "01_trueness_regression"
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    metadata_columns = {"donor", "specimen_type", "replicate_number", "parse_ok"}
    if not metadata_columns.issubset(canonical_all.columns):
        if "bloodSampleId" not in canonical_all.columns:
            raise ValueError("Canonical data need bloodSampleId or explicit donor/specimen metadata columns.")
        parsed_all = pd.DataFrame([parse_blood_sample_id(v) for v in canonical_all["bloodSampleId"]])
        for column in parsed_all.columns:
            canonical_all[column] = parsed_all[column].values
    else:
    ...
    input_csv = output_dir / "canonical_selected_input.csv"
    canonical_selected.to_csv(input_csv, index=False)

    with notebook_runtime_configuration(runtime_analytes, ac_limits, clia_limits):
        regression_result = run_pipeline(
            input_csv=input_csv,
            output_root=trueness_dir,
            analysis_groups=regression_groups,
    ...
    combined_xlsx = output_dir / "PROXIMA_Trueness_Regression_Output.xlsx"
    shutil.copy2(regression_result["excel_path"], combined_xlsx)

    summary_csv = output_dir / "PROXIMA_REGRESSION_SUMMARY.csv"
    summary = pd.concat(
        [bundle["metrics"] for bundle in regression_result["groups"].values()],
        ignore_index=True,
    ) if regression_result["groups"] else pd.DataFrame()
    summary.to_csv(summary_csv, index=False)

    app_config = {
        "build": "PROXIMA V7 regression-only",
        "selected_analytes": list(runtime_analytes),
        "regression_groups": {k: list(v) for k, v in regression_groups.items()},
        "groups_to_run": list(groups_to_run),
        "trueness_outlier_method": "Validated generalized ESD on raw replicate rows before donor averaging",
        ...
        "regression_metrics_position": regression_metrics_position,
        "apply_outlier_removal": bool(apply_outlier_removal),
        "global_flag_filter": "Always retain only global_flag == FALSE",
        "selected_donors": sorted(donor_selection) if donor_selection is not None else "all detected",
        "selected_specimen_types": sorted(specimen_selection) if specimen_selection is not None else "all detected",
        "detected_donor_count": int(canonical_all.loc[canonical_all["parse_ok"].fillna(False), "donor"].nunique()),
        "selected_donor_count": int(canonical_selected["donor"].nunique()),
        "bland_altman": "Removed/disabled in this build",
    }
    with (output_dir / "APP_RUN_CONFIGURATION.json").open("w", encoding="utf-8") as handle:
        json.dump(app_config, handle, indent=2)

    zip_path = run_root / "PROXIMA_REGRESSION_RESULTS.zip"
    if zip_path.exists():
        zip_path.unlink()
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as archive:
        for path in output_dir.rglob("*"):
